...
combine -M FitDiagnostics --saveNLL --numToysForShapes 1000 --verbose 2 -d mt/ws.root -m 600 --saveNormalizations --saveShapes --saveWithUncertainties --saveWorkspace -n test_output --plots  --redefineSignalPOIs r_ggH,r_bbH
...
do
    ...
    python $CMSSW_BASE/src/CombineHarvester/MSSMFull2016/scripts/plotMSSMLimits.py --logy --logx mssm_${p}H_mt.json -o mssm_mt_${p}H --process "${p}#phi" --title-right="41.5 fb^{-1}" --title-left="#mu#tau_{h}" --show "exp,obs" --plot-exp-points --use-hig-17-020-style --y-axis-max 1000 --y-axis-min .001
done
